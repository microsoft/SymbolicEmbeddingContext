...
class JavaEvaluator(BaseEvaluator):
    ...
    def __init__(
        ...
        # os.makedirs("temp/java/eval_repo", exist_ok=True)
        # os.chmod("temp/java/eval_repo", 0o777)
        if os.path.exists("temp/java/eval_repo/"):
            shutil.rmtree("temp/java/eval_repo/")
        ...
        # for dirpath, dirnames, filenames in os.walk("temp/java/eval_repo"):
    ...
    def evaluate(self, code: str) -> EvaluationData:
        ...
        # Run the tests
        try:
            java_command = f'export JAVA_HOME={self.java_home} && {self.mvn_path} test -Dtest="{self.evaluation_metadata.test_class_name}" -DfailIfNoTests=false -Dsurefire.failIfNoSpecifiedTests=false'
